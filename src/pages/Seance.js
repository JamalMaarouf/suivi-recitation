...
export default function Seance({  user, navigate , lang="fr" }) {
  ...
  const [vue, setVue] = useState('seance'); // seance | semaine
  ...
  // Consolidation séance : élèves vus aujourd'hui, triés par tomon desc, puis heure desc
  const elevesVusAujourdhui = eleves
    .filter(e => e.tomonAujourdhui > 0 || e.hizbAujourdhui > 0)
    .sort((a, b) => {
      const scoreA = a.tomonAujourdhui * 10 + a.hizbAujourdhui * 100;
      const scoreB = b.tomonAujourdhui * 10 + b.hizbAujourdhui * 100;
      if (scoreB !== scoreA) return scoreB - scoreA;
      // égalité → heure de dernière validation
      return new Date(b.derniereValidationAujourdhui||0) - new Date(a.derniereValidationAujourdhui||0);
    });

  const elevesNonVus = eleves
    .filter(e => e.tomonAujourdhui === 0 && e.hizbAujourdhui === 0)
  ...
  const elevesSemaine = eleves
    .filter(e => e.tomonSemaine > 0)
    .sort((a,b) => b.tomonSemaine - a.tomonSemaine);

  // Jours de la semaine avec activité
  const joursActifs = {};
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', gap:0, background:'#f0f0ec', borderRadius:10, padding:3, marginBottom:'1.25rem', width:'fit-content' }}>
        {[['seance','📋 Séance du jour'],['semaine','📅 Cette semaine']].map(([k,l])=>(
        ...
        <>
          {vue === 'seance' && (
            <>
                ...
                <>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:8, marginBottom:'1.5rem' }}>
                    {elevesVusAujourdhui.map((e, idx) => {
                      ...
                      const derniereHeure = e.derniereValidationAujourdhui
                        ? new Date(e.derniereValidationAujourdhui).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'})
                        : '';
                      return (
                        <div key={e.id} onClick={() => navigate('fiche', e)}
                          style={{ display:'flex', alignItems:'center', gap:12, padding:'14px', background:'#fff', border:`0.5px solid ${idx===0?'#EF9F27':'#e0e0d8'}`, borderRadius:12, cursor:'pointer', transition:'all 0.15s' }}
                          ...
                          <div style={{ flex:1 }}>
                            ...
                            <div style={{ fontSize:12, color:'#888', marginTop:2 }}>
                              {e.tomonAujourdhui > 0 && `${e.tomonAujourdhui} Tomon`}
                              {e.tomonAujourdhui > 0 && e.hizbAujourdhui > 0 && ' + '}
                              {e.hizbAujourdhui > 0 && `${e.hizbAujourdhui} Hizb complet`}
              ...
              {/* Détail des validations (toutes) */}
              {validationsAujourdhui.length > 0 && (
                <>
                  <div className="section-label">Détail des validations</div>
                            ...
                            <tr key={v.id} className={eleve?'clickable':''} onClick={()=>eleve&&navigate('fiche',eleve)}>
                              ...
                              <td>
                                {v.type_validation==='hizb_complet'
                                  ...
                                  :<span className="badge badge-blue">{v.nombre_tomon} Tomon{v.tomon_debut?` (T.${v.tomon_debut}→T.${v.tomon_debut+v.nombre_tomon-1})`:''}  · Hizb {v.hizb_validation||'—'}</span>}
                              </td>
              ...
              {/* Élèves non vus */}
              <div className="section-label">À voir aujourd'hui ({elevesNonVus.length})</div>
                  ...
                  return (
                    <div key={e.id} onClick={() => navigate('enregistrer', e)}
                      ...
                      <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:3 }}>
                        <span style={{ fontSize:12, fontWeight:600, color:urgence?'#A32D2D':'#888' }}>{e.jours!=null?`${e.jours}j`:'Jamais'}</span>
                        <div style={{ display:'flex', gap:2 }}>
                          {[1,2,3,4,5,6,7,8].map(n=><div key={n} style={{width:5,height:5,borderRadius:1,background:n<=e.etat.tomonDansHizbActuel?'#1D9E75':'#e8e8e0'}}/>)}
                        </div>
                      </div>
                    </div>
                  );
              ...
              <div style={{ display:'flex', gap:6, marginBottom:'1.5rem', flexWrap:'wrap' }}>
                {Object.entries(joursActifs).reverse().map(([jour, data]) => (
                  <div key={jour} style={{ background:'#fff', border:'0.5px solid #e0e0d8', borderRadius:10, padding:'10px 14px', minWidth:80, textAlign:'center' }}>
                    ...
                    {data.tomon > 0 && <div style={{ fontSize:13, fontWeight:600, color:'#378ADD' }}>{data.tomon}T</div>}
              ...
              {elevesSemaine.length === 0 ? <div className="empty">{t(lang,'aucune_activite')}</div> : (
                <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                  {elevesSemaine.map((e, idx) => {
                    const sl = scoreLabel(e.etat.points.total);
                    const pctMax = elevesSemaine[0].tomonSemaine;
                    return (
                      <div key={e.id} onClick={() => navigate('fiche', e)}
                        ...
                        <div style={{ flex:1 }}>
                          ...
                          <div style={{ height:5, background:'#e8e8e0', borderRadius:3, marginTop:6, overflow:'hidden' }}>
                            <div style={{ height:'100%', width:`${(e.tomonSemaine/pctMax)*100}%`, background:'#1D9E75', borderRadius:3 }} />
                          </div>
                        </div>
                        <div style={{ textAlign:'right' }}>
                          <div style={{ fontSize:16, fontWeight:700, color:'#1D9E75' }}>{e.tomonSemaine}</div>
                          <div style={{ fontSize:10, color:'#888' }}>Tomon</div>
                        </div>
                      </div>
                    );
                  })}
