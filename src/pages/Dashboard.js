...
function calcAlertes(eleves, allValidations) {
  const alertes = [];
  ...
  eleves.forEach(e => {
    ...
    const valsSemannePrec = vals.filter(v => new Date(v.date_validation) >= deuxSemaines && new Date(v.date_validation) < semaineDerniere);
    const tomonSemaine = valsSemaine.filter(v=>v.type_validation==='tomon').reduce((s,v)=>s+v.nombre_tomon,0);
    const tomonSemannePrec = valsSemannePrec.filter(v=>v.type_validation==='tomon').reduce((s,v)=>s+v.nombre_tomon,0);
    const hizbBloque = e.etat.enAttenteHizbComplet;
    const derniere = vals[0]?.date_validation;
    const joursSansRecit = joursDepuis(derniere);

    // Alerte stagnation
    if (tomonSemaine === 0 && tomonSemannePrec > 0) {
      alertes.push({ type: 'stagnation', eleve: e, msg: `Aucune récitation cette semaine (${tomonSemannePrec} Tomon la semaine dernière)`, color: C.red, bg: C.redBg, icon: '📉' });
    }
    // Alerte progression rapide
    if (tomonSemaine >= 6) {
      alertes.push({ type: 'rapide', eleve: e, msg: `Excellente semaine — ${tomonSemaine} Tomon récités`, color: C.green, bg: C.greenBg, icon: '🚀' });
    }
    // Hizb bloqué depuis longtemps
    if (hizbBloque && joursSansRecit > 7) {
      alertes.push({ type: 'hizb_bloque', eleve: e, msg: `Hizb ${e.etat.hizbEnCours} en attente de validation depuis ${joursSansRecit} jours`, color: C.amber, bg: C.amberBg, icon: '⏳' });
    }
    // Record personnel
    if (tomonSemaine > 0 && tomonSemaine > tomonSemannePrec && tomonSemannePrec > 0 && tomonSemaine >= tomonSemannePrec * 1.5) {
      alertes.push({ type: 'record', eleve: e, msg: `Record personnel — ${tomonSemaine} Tomon cette semaine !`, color: '#534AB7', bg: '#EEEDFE', icon: '⭐' });
    }
  });

  return alertes.sort((a,b) => {
    const order = { stagnation:0, hizb_bloque:1, record:2, rapide:3 };
...
export default function Dashboard({ user, navigate }) {
  ...
  const alertes = useMemo(() => calcAlertes(eleves, allValidations), [eleves, allValidations]);
  ...
  const exportExcel = () => {
    ...
    const rows = [['Rang','Prénom','Nom','Niveau','Instituteur','Hizb départ','Tomon départ','Hizb en cours','Tomon/Hizb','Total Tomon','Hizb complets','Pts Tomon','Pts Roboe','Pts Nisf','Pts Hizb','Score Total','Dernière récitation','Statut'],
      ...rang.map((e,idx)=>[idx+1,e.prenom,e.nom,e.niveau,e.instituteurNom,e.hizb_depart,e.tomon_depart,e.etat.hizbEnCours,e.etat.tomonDansHizbActuel,e.etat.tomonCumul,e.etat.hizbsComplets.size,e.etat.points.ptsTomon,e.etat.points.ptsRoboe,e.etat.points.ptsNisf,e.etat.points.ptsHizb,e.etat.points.total,e.derniere?formatDate(e.derniere):'Jamais',e.etat.enAttenteHizbComplet?'Attente Hizb':e.inactif?`Inactif (${e.jours}j)`:'Actif'])];
    ...
    setExportMsg('✓ Export téléchargé'); setTimeout(()=>setExportMsg(''),3000);
  };
  ...
  const backupJSON = async () => {
    ...
    setExportMsg('✓ Backup téléchargé'); setTimeout(()=>setExportMsg(''),3000);
  };

  const toggleCompare = (e) => {
    if(selectedEleves.find(s=>s.id===e.id)) setSelectedEleves(selectedEleves.filter(s=>s.id!==e.id));
    else if(selectedEleves.length<6) setSelectedEleves([...selectedEleves,e]);
  };

  const tabs = [{key:'general',label:'🏠 Général'},{key:'eleves',label:'👥 Élèves'},{key:'instituteurs',label:'👨‍🏫 Instituteurs'},...(user.role==='surveillant'?[{key:'rapport',label:'📊 Rapport'}]:[])];
    ...
    <div>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'1rem',flexWrap:'wrap',gap:8}}>
        ...
        <div style={{display:'flex',gap:6,flexWrap:'wrap',alignItems:'center'}}>
          ...
          <button onClick={()=>navigate('honneur')} style={{padding:'6px 10px',background:C.green,color:'#fff',border:'none',borderRadius:8,fontSize:11,cursor:'pointer',fontWeight:500}}>🏆 Honneur</button>
          <button onClick={()=>navigate('comparaison')} style={{padding:'6px 10px',border:`0.5px solid ${C.border}`,borderRadius:8,background:'#fff',fontSize:11,cursor:'pointer'}}>📈 Comparer</button>
        </div>
      </div>
      ...
      <div style={{display:'flex',gap:0,background:'#f0f0ec',borderRadius:10,padding:3,marginBottom:'1.25rem',width:'fit-content',flexWrap:'wrap'}}>
        {tabs.map(t=><div key={t.key} onClick={()=>setVue(t.key)} style={{padding:'7px 14px',borderRadius:8,fontSize:12,fontWeight:vue===t.key?600:400,cursor:'pointer',background:vue===t.key?'#fff':'transparent',color:vue===t.key?C.dark:C.muted,border:vue===t.key?`0.5px solid ${C.border}`:'none',transition:'all 0.15s',whiteSpace:'nowrap'}}>{t.label}</div>)}
      </div>
      ...
      {!loading && vue==='general' && (
        <>
          ...
          <div style={{background:'linear-gradient(135deg,#085041 0%,#1D9E75 100%)',borderRadius:16,padding:'1.5rem',marginBottom:'1.25rem',color:'#fff',position:'relative',overflow:'hidden'}}>
            ...
            <div style={{fontSize:12,opacity:0.7,marginTop:4,marginBottom:14}}>points cumulés</div>
            <div style={{display:'flex',gap:20,flexWrap:'wrap'}}>
              {[{v:totalTomon,l:'Tomon'},{v:totalHizb,l:'Hizb complets'},{v:eleves.length,l:'Élèves'},{v:instituteurs.length,l:'Instituteurs'}].map(k=>(
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,minmax(0,1fr))',gap:8,marginBottom:'1.25rem'}}>
            {[{val:stats.tomonSemaine||0,lbl:'Tomon semaine',color:C.green},{val:stats.hizbsCompletsMois||0,lbl:'Hizb ce mois',color:C.blue},{val:nbAttente,lbl:'Attente Hizb',color:C.amber},{val:nbInactifs,lbl:'Inactifs +14j',color:C.red}].map((k,i)=>(
          ...
          {alertes.length > 0 && (
            <>
              <div className="section-label">Alertes intelligentes ({alertes.length})</div>
              <div style={{display:'flex',flexDirection:'column',gap:6,marginBottom:'1.25rem'}}>
                {alertes.slice(0,6).map((a,i)=>(
                  <div key={i} onClick={()=>navigate('fiche',a.eleve)}
                    style={{display:'flex',alignItems:'center',gap:12,padding:'10px 14px',background:a.bg,borderLeft:`4px solid ${a.color}`,borderRadius:'0 10px 10px 0',cursor:'pointer'}}>
                    <span style={{fontSize:18}}>{a.icon}</span>
                    <div style={{flex:1}}>
                      <span style={{fontSize:13,fontWeight:500,color:a.color}}>{a.eleve.prenom} {a.eleve.nom}</span>
                      <span style={{fontSize:12,color:a.color,opacity:0.8}}> — {a.msg}</span>
                    </div>
            ...
            {[1,0,2].map(rank=>{
              ...
              return (
                <div key={e.id} onClick={()=>navigate('fiche',e)} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',cursor:'pointer',maxWidth:160}}>
                  ...
                  <div style={{fontSize:rank===0?16:13,fontWeight:700,color:pc[rank],margin:'4px 0'}}>{e.etat.points.total.toLocaleString()} pts</div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'1rem'}}>
            <div>
              ...
              {eleves.filter(e=>e.inactif).length===0?<div style={{padding:'1rem',background:C.greenBg,borderRadius:10,fontSize:13,color:'#085041',textAlign:'center'}}>Tous actifs ✓</div>:(
            ...
            <div>
              ...
              {nbAttente===0?<div style={{padding:'1rem',background:C.greenBg,borderRadius:10,fontSize:13,color:'#085041',textAlign:'center'}}>Aucun en attente ✓</div>:(
                <div style={{display:'flex',flexDirection:'column',gap:6}}>
          ...
          <div className="table-wrap">
            <table><thead><tr><th style={{width:'18%'}}>Date</th><th style={{width:'28%'}}>Élève</th><th style={{width:'30%'}}>Validation</th><th style={{width:'24%'}}>Validé par</th></tr></thead>
            <tbody>
              {allValidations.slice(0,8).map(v=>{const e=eleves.find(el=>el.id===v.eleve_id);return(
                <tr key={v.id} className={e?'clickable':''} onClick={()=>e&&navigate('fiche',e)}>
                  ...
                  <td>{v.type_validation==='hizb_complet'?<span className="badge badge-green">Hizb {v.hizb_valide} complet</span>:<span className="badge badge-blue">{v.nombre_tomon} Tomon{v.tomon_debut?` (T.${v.tomon_debut}→${v.tomon_debut+v.nombre_tomon-1})`:''}</span>}</td>
        ...
        <>
          {selectedEleves.length > 0 && (
            <div style={{display:'flex',alignItems:'center',gap:8,padding:'10px 14px',background:C.blueBg,border:`0.5px solid ${C.blue}`,borderRadius:10,marginBottom:'1rem'}}>
              <span style={{fontSize:12,color:'#0C447C'}}>{selectedEleves.length} élève{selectedEleves.length>1?'s':''} sélectionné{selectedEleves.length>1?'s':''}</span>
              <button onClick={()=>navigate('comparaison',selectedEleves)} style={{padding:'4px 12px',background:C.blue,color:'#fff',border:'none',borderRadius:6,fontSize:11,cursor:'pointer',marginLeft:'auto'}}>📈 Comparer</button>
              <button onClick={()=>setSelectedEleves([])} style={{padding:'4px 10px',border:`0.5px solid ${C.border}`,borderRadius:6,background:'#fff',fontSize:11,cursor:'pointer'}}>Effacer</button>
            </div>
          )}
          ...
          <div style={{background:'#fff',border:`0.5px solid ${C.border}`,borderRadius:12,padding:'1rem',marginBottom:'1rem'}}>
            <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:10}}>
              ...
              <select className="field-select" style={{flex:1,minWidth:130}} value={filtreInst} onChange={e=>setFiltreInst(e.target.value)}>
                <option value="tous">Tous instituteurs</option>
              ...
              <select className="field-select" style={{flex:1,minWidth:110}} value={filtreStatut} onChange={e=>setFiltreStatut(e.target.value)}>
                <option value="tous">Tous statuts</option><option value="actifs">Actifs</option><option value="inactifs">Inactifs</option><option value="attente">Attente Hizb</option>
              </select>
              <select className="field-select" style={{flex:1,minWidth:110}} value={filtreNiveau} onChange={e=>setFiltreNiveau(e.target.value)}>
                <option value="tous">Tous niveaux</option><option>Débutant</option><option>Intermédiaire</option><option>Avancé</option>
              </select>
            </div>
            <div style={{display:'flex',gap:6,flexWrap:'wrap',alignItems:'center'}}>
          ...
          {elevesFiltres.length===0?<div className="empty">Aucun élève.</div>:(
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))',gap:12}}>
              {elevesFiltres.map(eleve=>{
                ...
                return(
                  <div key={eleve.id}
                    style={{background:'#fff',border:`${isSelected?'2px':'0.5px'} solid ${isSelected?C.blue:eleve.etat.enAttenteHizbComplet?C.amber:eleve.inactif?C.red:C.border}`,borderRadius:14,padding:'1.25rem',cursor:'pointer',transition:'transform 0.15s',position:'relative'}}
                    onMouseEnter={ev=>ev.currentTarget.style.transform='translateY(-2px)'}
                    onMouseLeave={ev=>ev.currentTarget.style.transform='translateY(0)'}>
                    {/* Checkbox comparaison */}
                    <div onClick={e=>{e.stopPropagation();toggleCompare(eleve);}}
                    ...
                    <div onClick={()=>navigate('fiche',eleve)}>
                      ...
                      <div style={{fontSize:11,color:C.muted,marginBottom:6}}>Hizb {eleve.etat.hizbEnCours} · {eleve.etat.tomonCumul} Tomon · {eleve.etat.hizbsComplets.size} Hizb cplt</div>
                      <Bar8 done={eleve.etat.tomonDansHizbActuel} color={eleve.etat.enAttenteHizbComplet?C.amber:C.green}/>
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:8}}>
                        <div style={{fontSize:11,color:C.muted}}>{eleve.derniere?formatDateCourt(eleve.derniere):'Jamais'}</div>
            ...
            <div key={inst.id} style={{background:'#fff',border:`0.5px solid ${C.border}`,borderRadius:14,padding:'1.25rem',cursor:'pointer'}} onClick={()=>navigate('profil_instituteur',inst)}>
              <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:14}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:11,color:C.muted}}>{inst.nbEleves} élèves · Voir profil →</div>
                </div>
                <Medaille idx={idx}/>
              </div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:6,marginBottom:12}}>
                {[{l:'Tomon',v:inst.totalTomon,bg:C.blueBg,c:'#0C447C'},{l:'Hizb',v:inst.totalHizb,bg:C.greenBg,c:'#085041'},{l:'Inactifs',v:inst.nbInactifs,bg:inst.nbInactifs>0?C.redBg:'#f9f9f6',c:inst.nbInactifs>0?'#A32D2D':'#bbb'},{l:'Attente',v:inst.nbAttente,bg:inst.nbAttente>0?C.amberBg:'#f9f9f6',c:inst.nbAttente>0?'#633806':'#bbb'}].map(s=>(
              ...
              {inst.meilleur&&<div style={{borderTop:`0.5px solid ${C.border}`,paddingTop:10}}>
                ...
                <div style={{display:'flex',alignItems:'center',gap:8}}>
                  ...
                  <div style={{flex:1}}><div style={{fontSize:12,fontWeight:500}}>{inst.meilleur.prenom} {inst.meilleur.nom}</div><div style={{fontSize:11,color:C.muted}}>Hizb {inst.meilleur.etat.hizbEnCours} · {inst.meilleur.etat.points.total.toLocaleString()} pts</div></div>
                </div>
              </div>}
            </div>
          ))}
      ...
      {!loading && vue==='rapport' && user.role==='surveillant' && (
        <>
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,minmax(0,1fr))',gap:8,marginBottom:'1.5rem'}}>
            {[{val:totalPoints.toLocaleString(),lbl:'Score total école',color:C.green},{val:stats.hizbsCompletsMois||0,lbl:'Hizb ce mois',color:C.blue},{val:stats.tomonSemaine||0,lbl:'Tomon cette semaine',color:C.amber},{val:stats.recitationsMois||0,lbl:'Récitations ce mois',color:C.muted}].map((k,i)=>(
          ...
          <div className="table-wrap" style={{marginBottom:'1.5rem'}}>
            <table><thead><tr><th style={{width:'5%'}}>#</th><th style={{width:'20%'}}>Élève</th><th style={{width:'12%'}}>Niveau</th><th style={{width:'15%'}}>Instituteur</th><th style={{width:'10%'}}>Hizb</th><th style={{width:'8%'}}>Tomon</th><th style={{width:'8%'}}>Hizb cplt</th><th style={{width:'12%'}}>Score</th><th style={{width:'10%'}}>Statut</th></tr></thead>
            <tbody>
