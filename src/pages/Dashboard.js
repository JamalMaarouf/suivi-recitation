...
function NiveauBadge({ code, niveaux=[] }) {
...
export default function Dashboard({ user, navigate, goBack, lang, isMobile=false }) {
  ...
  const [vue, setVueLocal] = useState(initialVue || 'general');
  const setVue = (v) => { setVueLocal(v); if(setDashboardVue) setDashboardVue(v); };
              ...
              <div style={{background:'#fff',borderRadius:16,padding:'16px 12px 12px',
                ...
                <div style={{display:'flex',alignItems:'flex-end',justifyContent:'center',gap:6}}>
                  {podiumOrder.map(rank=>{
                    ...
                    return(
                      <div key={e.id} onClick={()=>navigate('fiche',e,{dashboardVue:vue})}
    ...
    <div>
              ...
              {elevesFiltres.map(eleve=>{
                ...
                return(
                  <div key={eleve.id} style={{background:'#fff',border:`${isSelected?'2px':'0.5px'} solid ${isSelected?C.blue:eleve.etat.enAttenteHizbComplet?C.amber:eleve.inactif?C.red:C.border}`,borderRadius:14,padding:'1.25rem',cursor:'pointer',transition:'transform 0.15s',position:'relative'}}
                    ...
                    <div onClick={()=>navigate('fiche',eleve,{dashboardVue:vue})}>
                      <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:12,paddingLeft:24}}>
                        <div style={{display:'flex',alignItems:'center',gap:10}}>
                          ...
                          <div>
                            <div style={{display:'flex',alignItems:'center',gap:5,flexWrap:'wrap'}}><div style={{fontSize:14,fontWeight:600}}>{eleve.prenom} {eleve.nom}</div><NiveauBadge code={eleve.code_niveau} niveaux={niveaux}/></div>
      ...
      {!loading&&vue==='rapport'&&user.role==='surveillant'&&(
        <>
          ...
          <div className="table-wrap" style={{marginBottom:'1.5rem'}}><table><thead><tr><th style={{width:'5%'}}>#</th><th style={{width:'22%'}}>{t(lang,'eleve')}</th><th style={{width:'14%'}}>{t(lang,'niveau')}</th><th style={{width:'15%'}}>{t(lang,'referent')}</th><th style={{width:'10%'}}>{t(lang,'hizb_en_cours')}</th><th style={{width:'8%'}}>{t(lang,'tomon_abrev')}</th><th style={{width:'10%'}}>{t(lang,'hizb_complets')}</th><th style={{width:'16%'}}>{t(lang,'score_total')}</th></tr></thead><tbody>{[...eleves].sort((a,b)=>b.etat.points.total-a.etat.points.total).map((e,idx)=>{const sl=scoreLabel(e.etat.points.total);return(<tr key={e.id} className="clickable" onClick={()=>navigate('fiche',e,{dashboardVue:vue})}><td><Medaille idx={idx}/></td><td><div style={{display:'flex',alignItems:'center',gap:6}}><Avatar prenom={e.prenom} nom={e.nom} size={24}/><span style={{fontSize:13}}>{e.prenom} {e.nom}</span></div></td><td><span className={`badge ${['Avancé','Advanced','متقدم'].some(v=>v.toLowerCase()===e.niveau?.toLowerCase())?'badge-green':['Intermédiaire','Intermediate','متوسط'].some(v=>v.toLowerCase()===e.niveau?.toLowerCase())?'badge-blue':'badge-amber'}`} style={{fontSize:10}}>{niveauTraduit(e.niveau,lang,t)}</span></td><td style={{fontSize:11,color:C.muted}}>{e.instituteurNom}</td><td style={{fontSize:12}}>Hizb {e.etat.hizbEnCours}</td><td><span className="badge badge-blue" style={{fontSize:10}}>{e.etat.tomonCumul}</span></td><td><span className="badge badge-green" style={{fontSize:10}}>{e.etat.hizbsComplets.size}</span></td><td><span style={{fontSize:13,fontWeight:700,color:sl.color}}>{e.etat.points.total.toLocaleString()}</span></td></tr>);})}</tbody></table></div>
        </>
      )}
    </div>
  );
