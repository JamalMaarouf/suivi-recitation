      ...
      <div style={{paddingBottom:80, background:'#f5f5f0', minHeight:'100vh'}}>
        ...
        <div style={{background:'linear-gradient(135deg,#085041,#1D9E75)',padding:'48px 16px 14px',position:'sticky',top:0,zIndex:100}}>
          <div style={{display:'flex', alignItems:'center', gap:10, padding:'14px 16px 8px'}}>
            <button onClick={()=>goBack?goBack():navigate('dashboard')} style={{background:'rgba(255,255,255,0.22)',border:'1px solid rgba(255,255,255,0.25)',borderRadius:10,padding:'0',color:'#fff',fontSize:20,cursor:'pointer',flexShrink:0,width:44,height:44,display:'flex',alignItems:'center',justifyContent:'center'}}>←</button>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8}}>
          <div className="field-group"><label className="field-lbl">{lang==='ar'?'المستوى':'Niveau'}</label>
            <select className="field-select" value={filterNiveau} onChange={e=>setFilterNiveau(e.target.value)}>
              ...
              {(niveaux||[]).map(n=><option key={n.code} value={n.code}>{n.code}{n.nom?' — '+n.nom:''}</option>)}
            </select>
          </div>
          ...
          <div className="field-group"><label className="field-lbl">{lang==='ar'?'الطالب':'Élève'}</label>
            <select className="field-select" value={filterEleve} onChange={e=>setFilterEleve(e.target.value)}>
              <option value="tous">{lang==='ar'?'كل الطلاب':'Tous les élèves'}</option>
