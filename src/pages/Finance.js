...
export default function Finance({ user, navigate, goBack, lang='fr', isMobile }) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setDepenses(r4.data||[]);
    } catch(e) { console.error(e); showMsg('error', lang==='ar'?'خطأ في التحميل':'Erreur de chargement'); }
    setLoading(false);
  };
  ...
  const exportPDF = () => {
    if (onglet === 'cotisations') {
      ...
      if (!w) { showMsg('warning', lang==='ar'?'يرجى السماح بالنوافذ المنبثقة للتصدير':'Autorisez les popups pour exporter'); return; }
