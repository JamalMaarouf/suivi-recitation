...
export default function RapportMensuel({ user, navigate, goBack, lang='fr', isMobile }) {
  ...
  const loadAll = async () => {
    ...
    const [
      ...
      supabase.from('recitations_sourates').select('*').eq('ecole_id',user.ecole_id).limit(3000).order('created_at', {ascending:false}),
...
function buildRapportHTML({
  ...
  `;
  const headerBand = `
    <div style="background:linear-gradient(135deg,#374151,#4B5563);color:#fff;
  ...
  const page2 = `
    <div class="rapport-page" style="${pageStyle}">
      ...
      <table style="width:100%;border-collapse:collapse;font-size:11px;">
        <thead>
          <tr style="background:linear-gradient(135deg,#374151,#4B5563);color:#fff;">
